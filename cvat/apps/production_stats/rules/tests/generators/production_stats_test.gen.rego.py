...
def eval_rule(scope, context, ownership, privilege, membership):
    ...
    rules = list(filter(lambda r: not r["limit"] or eval(r["limit"]), rules))  # nosec B307
...
def get_data(scope, context, ownership, privilege, membership):
    data = {
        "scope": scope,
        "auth": {
            "user": {"id": random.randrange(0, 100), "privilege": privilege},  # nosec B311
            "organization": (
                {
                    "id": random.randrange(100, 200),  # nosec B311
                    "owner": {"id": random.randrange(200, 300)},  # nosec B311
...
def is_valid(scope, context, ownership, privilege, membership):
    # NOTE: the upstream generators also drop `scope == "list"` rows whose
    # ownership is not "None". That check compares against the string "None"
    # while OWNERSHIPS only ever contains "none", so it would silently drop
    # every list-scope case. This package has no owned resource, so the check
